...
const AI_PROFILE = path.join(CONTENT, 'ai', 'profile.md');
...
const profileText = exists(AI_PROFILE) ? read(AI_PROFILE).trim() : '';
...
const contact=[]; if(CONFIG.github) contact.push(`<a class="btn btn-primary" href="${attr(CONFIG.github)}" target="_blank" rel="noopener">GitHub ↗</a>`); if(CONFIG.email) contact.push(`<a class="btn btn-ghost" href="mailto:${attr(CONFIG.email)}">Email</a>`); for(const [k,v] of Object.entries(CONFIG.social||{})) if(v) contact.push(`<a class="btn btn-ghost" href="${attr(v)}" target="_blank" rel="noopener">${esc(k)}</a>`); if(CONFIG.resume) contact.push(`<a class="btn btn-ghost" href="${attr(CONFIG.resume)}" target="_blank" rel="noopener">CV ↗</a>`); if(CONFIG.phone) contact.push(`<a class="btn btn-ghost" href="tel:${attr(CONFIG.phone)}">Phone</a>`); if(!contact.length) contact.push(`<a class="btn btn-primary" href="${basePath}/projects/">Browse the work ↗</a>`);
